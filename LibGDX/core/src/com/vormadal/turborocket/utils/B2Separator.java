...
public class B2Separator{
	...
	public static void seperate(Body body, FixtureDef fixtureDef, Vector2[] verticesVec, float scale){
		...
		for (int i=0; i<n; i++) {
			vec[i] = new Vector2(verticesVec[i].x*scale,verticesVec[i].y*scale);
		}


		Vector2[][] figsVec=calcShapes(vec);
		n=figsVec.length;

		for (int i=0; i<n; i++) {
			vec=figsVec[i];
			m=vec.length;
			verticesVec= new Vector2[m];
			for (j=0; j<m; j++) {

				verticesVec[j] = new Vector2(vec[j].x/scale,vec[j].y/scale);
			}
	...
	private static Vector2[][] calcShapes(Vector2[] verticesVec){
		Vector2[] vec;
		int i,n,j = 0;
		...
		ArrayList<Vector2[]> figsVec = new ArrayList<Vector2[]>();
...
//						vec2.push(hitV);
					...
					queue.addLast(vec1.toArray( new Vector2[vec1.size()]));
					queue.addLast(vec2.toArray(new Vector2[vec2.size()]));
					queue.remove(0);
		...
		return figsVec.toArray(new Vector2[figsVec.size()][]);
	}
